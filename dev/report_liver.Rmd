...
results_folder <- "..\\data\\adhoc\\liver\\res\\"



model_name <- "all  predictors model"


save_results = TRUE

save_results_folder <- paste0(results_folder, "output_report\\")
...
for (i in 1:length(n_ex_files)){
  print(i)
  load(paste0(results_folder, n_ex_files[i]))
...
for (i in 1:length(ratio_recall_files)){
  load(paste0(results_folder, ratio_recall_files[i]))
...
for (i in 1:length(stat_data_all_files)){
  load(paste0(results_folder, stat_data_all_files[i]))
...
for (i in 1:length(var_imp_files)){
  load(paste0(results_folder, var_imp_files[i]))
...
# select datasets with more tan 10 examples
datasets <- n_ex[n_ex$n1 >10,]$dataset
ratio_recall_data_all <- ratio_recall_data_all[ratio_recall_data_all$dataset %in% datasets, ]
stat_data_all <- stat_data_all[stat_data_all$dataset %in% datasets, ]
var_imp_all <- var_imp_all[var_imp_all$dataset %in% datasets, ]
```
